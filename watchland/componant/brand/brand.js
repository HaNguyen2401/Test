...
for (let key in element__brand_pic) {
    if (!isNaN(key)) {
        ...
        element__brand_pic[key].addEventListener("mouseenter", function() {
            element__brand_pic[key].src = element__brand_pic[key].src.replace("s/brand","s/brand-red")
            
        });


        element__brand_pic[key].addEventListener("mouseleave", function() {
            element__brand_pic[key].src = element__brand_pic[key].src.replace("s/brand-red","s/brand")
        });
    }
